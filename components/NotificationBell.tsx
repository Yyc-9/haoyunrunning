  ...
  return <>
    ...
    {mounted && createPortal(<dialog ref={dialog} onCancel={close} onClose={close} aria-label="報名通知" style={{ width: 'min(28rem, calc(100% - 2rem))' }} className="m-auto max-h-[85dvh] overflow-y-auto rounded-2xl border border-black/10 bg-white p-0 text-black shadow-2xl backdrop:bg-black/30">
      <div className="sticky top-0 z-10 border-b bg-white p-5">
        <div className="flex items-center justify-between gap-3"><div><h2 className="text-lg font-black">通知</h2><p className="mt-1 text-xs text-apple-gray-500">已讀不代表已完成核對或補件</p></div><button type="button" aria-label="關閉通知" onClick={close} className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-full border"><X className="h-5 w-5" /></button></div>
        <button type="button" disabled={busy || !feed?.items.some(n => !n.read_at)} onClick={() => void markRead(feed?.items.filter(n => !n.read_at).map(n => n.id) ?? [])} className="mt-3 min-h-11 rounded-full border px-4 py-2 text-xs font-bold disabled:opacity-40">將目前通知標為已讀</button>
      </div>
      ...
      <div className="p-5"><Link href={feed?.staff ? '/notifications?view=staff' : '/notifications'} onClick={close} className="text-sm font-bold underline">查看報名待辦</Link></div>
