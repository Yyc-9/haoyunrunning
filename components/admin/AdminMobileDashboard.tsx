...
export default function AdminMobileDashboard({ data, runAction, updatingId, actionMessage, actionError }: Props) {
  ...
  function navigate(nextView: MobileView) {
    ...
    if (view === 'products' && nextView !== 'products') {
      ...
      if (productEditState.dirty && !window.confirm(language === 'en' ? 'This product has unsaved changes. Discard them and leave?' : '商品仍有未儲存變更，確定要放棄並離開嗎？')) return false
    ...
    <section ref={dashboardRef} className="admin-mobile-dashboard" aria-label="手機管理員後台">
      <header className="admin-mobile-topbar">
        <Link href="/" className="admin-mobile-brand" aria-label="好運跑班，返回首頁" onClick={(event) => {
          if (view === 'products' && (productEditState.busy || (productEditState.dirty && !window.confirm(language === 'en' ? 'This product has unsaved changes. Discard them and return home?' : '商品仍有未儲存變更，確定要放棄並返回首頁嗎？')))) event.preventDefault()
        }}>
