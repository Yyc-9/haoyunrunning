...
alter table public.signup_leads add column if not exists course_slug text not null default '';
alter table public.signup_leads add column if not exists external_submission_id text;
alter table public.signup_leads add column if not exists form_submitted_at timestamptz;
...
create or replace function public.approve_course_enrollment(p_lead_id uuid, p_review_note text default '')
returns jsonb
language plpgsql
security invoker
set search_path = ''
...
begin
  ...
  where id = p_lead_id and source = 'course_payment'
  for update;

  if not found then raise exception 'course enrollment not found'; end if;
  if v_lead.status = 'approved' then return to_jsonb(v_lead); end if;
  if coalesce(v_lead.course_slug, '') = '' then raise exception 'course slug is missing'; end if;
  ...
  if v_paid_count >= 40 then raise exception 'course capacity reached'; end if;

  update public.signup_leads
  set status = 'approved', reviewed_at = now(), review_note = coalesce(p_review_note, ''), updated_at = now()
  where id = p_lead_id
