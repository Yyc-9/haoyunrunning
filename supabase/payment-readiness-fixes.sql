...
begin
  ...
  insert into public.shop_orders (
    ...
    item_count, status, payment_method, payment_reference,
    payment_account_id, payment_account_label, inventory_reserved
  ) values (
    p_order_number, p_user_id, p_customer_name, p_contact, p_email, p_fulfillment_note,
    0, 'pending_transfer', 'bank_transfer', p_order_number,
    v_account.id, coalesce(v_account.label, ''), true
  ...
  if v_account.id is not null then
    update public.shop_payment_accounts set last_assigned_at = now() where id = v_account.id;
  end if;
