...
export default function CoachLayout({ children }: { children: React.ReactNode }) {
  ...
  if (isLoading) return <main className="flex min-h-screen items-center justify-center gap-3 pt-24"><Loader2 aria-hidden="true" className="h-5 w-5 animate-spin" /><p role="status">正在確認教練身分</p></main>
  if (user?.role === 'coach' || user?.role === 'admin') return children
  return (
    <main className="min-h-screen bg-apple-gray-50 px-5 pb-16 pt-32">
