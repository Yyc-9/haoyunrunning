  ...
  return (
    <div ref={topRef} className="scroll-mt-28">
      ...
      <div className="px-5 py-6 sm:px-8 sm:py-8">
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                {coaches.map((coach) => (
                  <div key={coach.name} className="min-w-0 rounded-lg border border-black/10 bg-apple-gray-50 p-4">
                    ...
                    <p className="mt-1 text-sm font-semibold leading-6 text-apple-gray-700">{formatCourseWeekday(course.name)}</p>
                  </div>
                ))}
              </div>
            </div>
                ...
                {invoiceDeliveryOptions.map((option) => (
                  <label key={option} className={`flex cursor-pointer items-start gap-3 rounded-lg border px-4 py-3 text-sm font-bold leading-6 ${form.invoiceDelivery === option ? 'border-blue-500 bg-blue-50 text-blue-900' : 'border-black/10'}`}>
                    <input type="radio" name="invoiceDelivery" checked={form.invoiceDelivery === option} onChange={() => {
                      ...
                      if (option === '寄至本次的電子信箱或 Gmail 信箱') update('invoiceDetail', userEmail)
        ...
        <div className="mobile-registration-actions mt-8 flex gap-3 border-t border-black/10 pt-5">
