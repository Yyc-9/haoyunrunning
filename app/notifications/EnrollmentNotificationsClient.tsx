  ...
  return <main className="mx-auto min-h-screen max-w-7xl px-4 pb-24 pt-28 sm:px-6 sm:pt-32">
    ...
    {authLoading ? <p role="status">正在確認登入狀態…</p> : !user ? <section className="apple-card p-8"><Bell className="mb-4 h-7 w-7" /><h2 className="text-lg font-bold">登入後查看你的報名通知</h2><p className="my-4 text-sm text-apple-gray-500">登入後會保留目前這筆報名連結。</p><button className="apple-button-primary" onClick={() => setLogin(true)}>登入帳號</button></section> : !payload ? !error && <p role="status" className="flex items-center gap-2"><Loader2 className="h-4 w-4 animate-spin" />正在讀取報名…</p> : id ? <>
      ...
      {!enrollment ? <p className="rounded-xl border p-6">找不到這筆報名，或目前帳號沒有查閱權限。</p> : <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1.3fr)_minmax(18rem,.8fr)]">
        <section className="overflow-hidden rounded-2xl border border-black/10 bg-white shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3 border-b p-5"><div><p className="text-xs text-apple-gray-500">{enrollment.season_name} · {enrollment.preferred_course}</p><h2 className="mt-1 text-xl font-black">{staff ? `${enrollment.name}的報名` : '報名進度'}</h2></div><Status value={enrollment.status} /></div><div className="p-5 sm:p-6"><dl className="grid grid-cols-2 gap-5 text-sm"><div><dt className="text-apple-gray-500">應繳金額</dt><dd className="mt-1 font-bold">{enrollment.amount_text || '待確認'}</dd></div><div><dt className="text-apple-gray-500">帳號後五碼</dt><dd className="mt-1 font-bold">{enrollment.transfer_last_five || '尚未回報'}</dd></div><div><dt className="text-apple-gray-500">匯款日期</dt><dd className="mt-1 font-bold">{enrollment.transfer_date || '尚未回報'}</dd></div><div><dt className="text-apple-gray-500">報名日期</dt><dd className="mt-1 font-bold">{new Date(enrollment.created_at).toLocaleDateString('zh-TW', { timeZone: 'Asia/Taipei' })}</dd></div></dl>
          {enrollment.student_review_message && <div className="mt-6 rounded-xl border border-amber-200 bg-amber-50 p-4"><h3 className="text-sm font-bold">{enrollment.status === 'rejected' ? '請補充以下資料' : '前次補充說明'}</h3><p className="mt-2 whitespace-pre-wrap break-words text-sm leading-7">{enrollment.student_review_message}</p></div>}
          {staff && enrollment.notes && <div className="mt-5 text-sm"><h3 className="font-bold">學生備註</h3><p className="mt-2 whitespace-pre-wrap break-words leading-6 text-apple-gray-600">{enrollment.notes}</p></div>}
        ...
        <section className="rounded-2xl border border-black/10 bg-white p-5"><h2 className="font-black">{staff ? '補件與寄送紀錄' : '補充紀錄'}</h2>{!payload.followups.length ? <p className="mt-4 text-sm text-apple-gray-500">目前沒有補件紀錄。</p> : payload.followups.map(f => <article key={f.id} className="mt-5 border-t pt-4 text-sm"><p className="font-bold">{supplementReasons[f.reason]} · {f.responded_at ? '已結束' : '待學生補充'}</p><p className="mt-2 whitespace-pre-wrap break-words leading-6">{f.student_message}</p>{f.student_reply && <div className="mt-3 rounded-lg bg-apple-gray-100 p-3"><strong>學生回覆</strong><p className="mt-1 whitespace-pre-wrap break-words">{f.student_reply}</p></div>}{staff && <><p className="mt-3 whitespace-pre-wrap break-words text-xs text-apple-gray-500">內部備註：{f.internal_note || '無'}</p><p className="mt-3 text-xs font-bold">{emailLabels[f.email_status || 'pending']}</p>{f.email_error && <p className="mt-1 text-xs text-amber-800">{f.email_error}</p>}{!enrollment.archived && !f.responded_at && ['pending','failed','skipped','sending'].includes(f.email_status || '') && <button disabled={busy} onClick={() => void action({ action: 'retry_email', requestId: f.id }, 'POST')} className="mt-3 inline-flex min-h-11 items-center gap-2 rounded-full border px-4 text-xs font-bold disabled:opacity-50"><RotateCcw className="h-3 w-3" />重試寄送</button>}</>}<p className="mt-3 text-[11px] text-apple-gray-500">{new Date(f.created_at).toLocaleString('zh-TW', { timeZone: 'Asia/Taipei' })} · UTC+8</p></article>)}</section>
      </div>}
    </> : <section className="overflow-hidden rounded-2xl border border-black/10 bg-white shadow-sm"><div className="flex flex-wrap gap-2 border-b p-4">{[['all','全部'],['pending_review','待核對'],['pending_transfer','待回報'],['rejected','待補充'],['approved','已入帳']].map(([value,label]) => <button key={value} aria-pressed={filter === value} onClick={() => { setPage(0); setFilter(value) }} className={`min-h-11 rounded-full border px-4 text-sm font-bold ${filter === value ? 'bg-black text-white' : ''}`}>{label}</button>)}</div><p className="px-5 pt-4 text-xs text-apple-gray-500">第 {page + 1} 頁 · 本頁 {payload.enrollments.length} 筆 · 已依所選狀態篩選</p>{payload.enrollments.filter(e => filter === 'all' || filter === e.status).map(e => <Link key={e.id} href={notificationHref(e.id, staff)} className="flex flex-wrap items-center justify-between gap-3 border-b p-5 hover:bg-apple-gray-100"><div><p className="font-bold">{staff ? `${e.name} · ` : ''}{e.preferred_course}</p><p className="mt-1 text-xs text-apple-gray-500">{e.season_name} · {e.amount_text} {e.archived ? '· 已封存' : ''}</p></div><Status value={e.status} /></Link>)}{!payload.enrollments.some(e => filter === 'all' || filter === e.status) && <p className="p-8 text-center text-sm text-apple-gray-500">本頁沒有符合條件的報名。</p>}<div className="flex gap-3 p-4"><button disabled={!page} onClick={() => setPage(p => p - 1)} className="rounded-full border px-4 py-2 text-sm disabled:opacity-40">上一頁</button><button disabled={!payload.hasMore} onClick={() => setPage(p => p + 1)} className="rounded-full border px-4 py-2 text-sm disabled:opacity-40">下一頁</button></div></section>}
...
function StaffForm({ enrollment: e, followups, busy, onAction }: FormProps) {
  ...
  return <form className="mt-6 space-y-4 border-t pt-5" onSubmit={event => { event.preventDefault(); const signature = JSON.stringify([reason,message,internalNote,e.status,e.payment_submitted_at]); if (signature !== requestBody.current) { requestId.current = crypto.randomUUID(); requestBody.current = signature } void onAction({ requestId: requestId.current, enrollmentId: e.id, reason, message, internalNote, expectedStatus: e.status, expectedSubmittedAt: e.payment_submitted_at }, 'POST') }}>
    ...
    <button type="button" onClick={() => setMessage(supplementTemplates[reason])} className="text-xs font-bold underline">帶入常用說明</button>
